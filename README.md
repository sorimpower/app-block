...
- 앱 차단 화면에 들어갈 때 백그라운드에서 조회해 실제 아이콘을 표시하는 앱별 차단 목록과 시스템 안전 영역을 고려한 하단 메뉴 바
...
- 식사 사진을 원본의 절반 크기(긴 변 최대 2,048px)로 샘플링해 메모리 사용을 줄이고, 삭제·수정 후 참조되지 않는 사진·썸네일 파일을 정리하는 로컬 사진 저장소 관리
- 마운자로 투여일·용량·부작용 기록, 월 달력의 주사 배지, 최신 기록에서 바꿀 수 있는 알림 ON/OFF 및 1~4주 반복 주기 설정
- 식사·마운자로 기록의 수정·삭제와 사진까지 함께 삭제됨을 알리는 삭제 확인 창
- 법원경매정보 API에서 서울·감정가 10억 이상·진행 중·아파트 경매를 직접 수집해 공통 보라·핑크 요약 카드, 조건 재검증, 신규 배지, 오프라인 캐시와 함께 보여주는 경매 목록
- 부동산 경매의 아파트명 검색, 감정가·매각기일·유찰 횟수 범위 필터, 감정가·최저가·매각기일·유찰 횟수 정렬, 로컬 관심 사건 저장과 지역·아파트명 기반 네이버지도 검색
- 경매 화면에서 Room 캐시를 즉시 표시하고 한국 시간 오전 9시 이후 하루 한 번만 자동 동기화하며, 실패 시 30분 뒤 재시도하고 당겨서 새로고침은 법원 API를 즉시 호출
- 전날과 오늘의 진행 중 목록 비교로 종료 사건을 별도 캐시·목록에서 확인하며 낙찰·취하 여부를 단정하지 않는 상태 안내
- 새 법원 검색 결과에서 사라진 사건은 이전 Room 스냅샷과 비교해 종료 이력에 한 번만 누적
- 종료 사건 카드에서 로컬 종료 기록을 개별 삭제하고 연결된 관심 사건도 함께 정리
- 서울 날짜 기준 오늘 처음 발견된 사건에만 `신규` 배지를 표시
- 사건번호를 탭해 `타경` 뒤 숫자만 클립보드에 복사하고 사건 카드의 아파트 이름을 눌러 법원 경매 조회 사이트를 여는 기능
- 긴 경매 비고는 2줄로 요약하고 비고 박스를 탭해 원문 전체 보기·접기
- 진행 중 사건의 매각일·시간과 사건·가격·법원·주소·비고를 휴대폰 캘린더 일정 작성 화면에 자동 입력
- 사건 카드에서 법원 사건상세·현황조사서·감정평가 요약·매각물건명세서 정보를 자동 수집해 ChatGPT 예비 권리분석을 실행하고 결과를 로컬 캐시
- 예산·서울 선호 구·최소 할인율·허용 위험도·점유 허용 여부·자유 조건을 저장하고 WorkManager가 매일 오전 새로 추가된 미분석 사건만 최대 5건 AI 분석해 최대 3건의 예비 추천을 로컬 알림으로 제공
...
- `feature/auction/data`: 법원경매정보 검색·문서 API 직접 조회, 원본 구성행 병합, 종료 사건 감지, Room 오프라인 캐시, AI 설정·분석 저장
- `feature/auction/domain/AuctionModels.kt`: 경매 조건·가격·기일·매각시간 정규화
- `feature/auction/presentation`: 경매 목록 상태, 새로고침과 Compose 카드 UI
- `feature/auction/reminder`: WorkManager 기반 매일 아침 AI 분석 예약과 로컬 추천 알림
...
- [경매 AI 권리분석 설정](docs/auction-ai-setup.md)

현재 앱에는 앱 차단, 건강 기록과 부동산 경매 목록이 구현되어 있습니다. 하단 메뉴는 홈·차단·기록·부동산 경매·더보기의 다섯 개 탭으로 고정하며, 설정과 미래 기능은 더보기와 기능 홈에서 확장합니다. 경매 목록은 진행 중·관심·종료 사건을 필터·정렬해 보여주며 마지막 갱신 시각, 신규 사건, 네트워크 실패 시 최근 캐시를 함께 제공합니다. 건강 기록의 세부 표시 단위 설정과 Health Connect는 명세의 후속 단계로 남겨두었습니다.
