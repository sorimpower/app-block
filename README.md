...
차단 화면에는 자동 종료 타이머가 없습니다. 시스템 뒤로 가기와 **홈으로 돌아가기**는 홈 화면으로 이동합니다. **정말 사용해야 해요**를 선택한 뒤 화면에 표시된 100자를 정확히 따라 입력하면 해당 앱의 이번 실행만 허용되고 원래 앱이 다시 열립니다. 다른 앱이나 홈으로 이동하면 허용 세션이 끝나므로 다음 실행부터 다시 차단됩니다.
...
- `data/BlockerRepository.kt`: DataStore에 차단 앱, 앱별 조건 할당, 다중 차단·해제 조건, 일일 실행 카운트, 문구, 시작 화면, 비밀번호 해시와 1회성 허용 상태 저장
