...
- 경매 화면에서 Room 캐시를 즉시 표시하고 한국 시간 오전 9시 이후 하루 한 번만 자동 동기화하며, 실패 시 30분 뒤 재시도하고 당겨서 새로고침은 즉시 강제 동기화
- 전날과 오늘의 진행 중 목록 비교로 종료 사건을 별도 캐시·목록에서 확인하며 낙찰·취하 여부를 단정하지 않는 상태 안내
- 종료 사건은 페이지당 100건씩 조회하고 갱신 시각이 같으면 Room 스냅샷 재저장을 생략하는 동기화 최적화
...
- `feature/auction/data`: Google Apps Script의 진행 중·종료 사건 API 전체 페이지 조회, Room 오프라인 캐시와 관심 사건 저장
