...
object MorningDigestScheduler{
    ...
    fun scheduleAll(context:Context){cancelLegacy(context);InsightDigestSlot.entries.forEach{scheduleSlot(context,it)}}
...
object InsightReminderScheduler{
    ...
    fun schedule(context:Context,values:List<PhoneInsightEntity>){values.forEach{value->val trigger=value.dueAt?:return@forEach;if(trigger<=System.currentTimeMillis())return@forEach;val intent=Intent(context,InsightDueReceiver::class.java).putExtra(EXTRA_ID,value.id);val pending=PendingIntent.getBroadcast(context,value.id.hashCode(),intent,PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE);val alarm=context.getSystemService(AlarmManager::class.java);if(Build.VERSION.SDK_INT<31||alarm.canScheduleExactAlarms())alarm.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,trigger,pending)else alarm.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,trigger,pending)}}
}
...
internal object PhoneInsightNotifier{
    ...
    fun showOne(context:Context,value:PhoneInsightEntity){if(!allowed(context))return;manager(context).notify(value.id.hashCode(),builder(context).setContentTitle("지금 챙길 시간이에요").setContentText(value.title).setStyle(NotificationCompat.BigTextStyle().bigText(listOf(value.title,value.description).filter(String::isNotBlank).joinToString("\n"))).build())}
